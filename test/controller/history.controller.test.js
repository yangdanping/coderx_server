...
test('getUserHistory: loads list and count, sanitizes non-banned content, masks banned, sets Result.success payload', async () => {
  ...
  const historyList = [
    { id: 1, title: 'Keep', content: longHtml, status: 0 },
    { id: 2, title: 'Will mask', content: 'x', status: 1 },
  ];
  ...
  assert.deepEqual(ctx.body, {
    ...
    data: {
      result: [
        { id: 1, title: 'Keep', content: expectedPlain, status: 0 },
        { id: 2, title: '文章已被封禁', content: '文章已被封禁', status: 1 },
      ],
      total: 77,
      pageNum: 3,
      pageSize: 10,

...
test('article media routes are registered before the dynamic article route', () => {
  const getRoutePaths = articleRouter.stack.filter((layer) => layer.methods.includes('GET')).map((layer) => layer.path);
...
test('random TOC article route is registered before the dynamic article route', () => {
  const getRoutePaths = articleRouter.stack.filter((layer) => layer.methods.includes('GET')).map((layer) => layer.path);

  const randomTocIndex = getRoutePaths.indexOf('/article/random/toc');
  const articleDetailIndex = getRoutePaths.indexOf('/article/:articleId');

  assert.ok(randomTocIndex >= 0);
  assert.ok(randomTocIndex < articleDetailIndex);
});

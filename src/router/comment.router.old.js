...
/* ★获取单个评论接口(用于点赞和修改评论后)---------------------------------- */
commentRouter.get('/:commentId', commentController.getCommentByIdOld);

/* ★<用户对文章评论>的实现---------------------------------- */
commentRouter.post('/', verifyAuth, verifyStatus, commentController.addCommentOld);

/* ★<用户对评论点赞>的实现---------------------------------- */
commentRouter.post('/:commentId/like', verifyAuth, commentController.likeCommentOld);
